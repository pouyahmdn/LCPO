...
class MBCD:
    ...
    def update_metrics(self, obs, action, reward, next_obs, done):
        ...
        for i in range(self.num_models):
            ...
            model_means[:, :, 1:] += obs
            self.log_prob[i], self.var_mean[i], self.mean[i], self.variance[i] = self.get_logprob2(true_output, model_means, model_vars)
        ...
        new_model_log_pdf = -1/2 * ((self.state_dim+1)*np.log(2*np.pi) +
                                    ...
                                    (np.power(true_output-(true_output+self.num_stds*np.sqrt(self.variance[self.current_model])), 2) / self.variance[self.current_model]).sum(-1))
