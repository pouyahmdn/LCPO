...
from agent.core_alg.core_ppo import proximal_policy_optimization
...
def locoprpo(
        ...
        actions_torch: torch.Tensor, adv_torch: torch.Tensor, entropy_factor: float, ppo_kl_in: float,
        kappa: float, ppo_iters: int, ppo_clip: float, global_states_torch: torch.Tensor,
        ood_loss: torch.nn.CrossEntropyLoss, monitor: SummaryWriter, it: int) -> Tuple[float, float, float]:
    pg_loss_s = []
    entropy_s = []
    ...
    with torch.no_grad():
        ...
        log_pi_global_before = policy_net.log_pi(global_states_torch)
    ...
    for i in range(ppo_iters):
        ...
        q_global = policy_net.forward(global_states_torch[idx_s])
        memory_loss = ood_loss(q_global.view((-1, q_global.shape[-1])),
                               pi_global_before[idx_s].view((-1, q_global.shape[-1])))
        ...
        loss = pg_loss - entropy_factor * entropy_mean + kappa * memory_loss

        approx_kl = get_kl(log_pi_before[idx_s], pi_before[idx_s], log_pi).mean().item()
        if approx_kl > 1.5 * ppo_kl_in:
            break
    ...
    log_pi_new = policy_net.log_pi(global_states_torch)
    kl_ood = get_kl(log_pi_global_before, pi_global_before, log_pi_new)
    monitor.add_scalar('lpg/kl_out_of_d', kl_ood.mean().item(), it)
...
def train_lcppo(value_net: FullyConnectNN, policy_net: FCNPolicy, net_opt_p: torch.optim.Optimizer,
                net_opt_v: torch.optim.Optimizer, net_loss: torch.nn.MSELoss, local_loss: torch.nn.CrossEntropyLoss,
                device: torch.device, actions_np: np.ndarray, next_obs_np: np.ndarray, rewards_np: np.ndarray,
                obs_np: np.ndarray, terms_np: np.ndarray, truncs_np: np.ndarray, gamma: float, lam: float,
                entropy_factor: float, ppo_kl_in: float, kappa: float, ppo_iters: int, ppo_clip: float,
                ood_obs_np: np.ndarray, monitor: SummaryWriter, it: int) -> Tuple[float, float, float, np.ndarray,
    ...
    if len(ood_obs_np) > 0:
        # locally constrained proximal policy optimization
        pg_loss, entropy, log_pi_min = locoprpo(policy_net, net_opt_p, obs_torch, actions_torch, adv_torch,
                                                entropy_factor, ppo_kl_in, kappa, ppo_iters, ppo_clip,
                                                ood_obs_torch, local_loss, monitor, it)
    else:
        # advantage actor critic
        pg_loss, entropy, log_pi_min = \
            proximal_policy_optimization(policy_net, net_opt_p, obs_torch, actions_torch, adv_torch, entropy_factor,
                                         ppo_kl_in, ppo_iters, ppo_clip, monitor, it)
