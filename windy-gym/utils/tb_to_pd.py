...
def tabulate_events(run_dir: str):
    ...
    out = {}
    steps = list(set([e.step for e in summary_iterator.Scalars(tags[0])]))
    assert min(steps) == 0
    max_steps = max(steps) + 1
    out['_wallclock_min'] = np.ones(max_steps) * np.inf
    out['_wallclock_max'] = np.ones(max_steps) * -np.inf
    for tag in tags:
        # MBCD tags are repeated many times, skipping implementing that for now
        if tag[:5] == 'MBCD/' or tag == 'Multi/index_model':
            continue
        out[tag] = np.ones(max_steps) * np.nan
        for event in summary_iterator.Scalars(tag):
            if not np.isnan(out[tag][event.step]):
                import pdb
                pdb.set_trace()
